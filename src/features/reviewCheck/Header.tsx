...
const Header = React.memo(() => {
    ...
    const setIsMultiSelecting = useBoundStore(
        (state) => state.setIsMultiSelecting
    )
    const clearRecentTwoIndexes = useBoundStore(
        (state) => state.clearRecentTwoIndexes
    )
    ...
    return (
        ...
        <Box className="flex gap-6 p-3 h-[70px] items-center">
            ...
            <Box className="flex">
                ...
                />
                <CircleIcon
                    sx={{ backgroundColor: { color }, paddingRight: "12px" }}
                    fontSize="small"
                />
            </Box>
        </Box>
