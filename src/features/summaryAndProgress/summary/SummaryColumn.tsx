...
const SummaryColumn = memo(
    ({ inProgressStatus }: { inProgressStatus: InProgressStatus }) => {
        ...
        const filteredArray = flattenedArray.filter(
            (progress) =>
                ...
                progress.inProgressStatus === inProgressStatus
        )
    ...
    return (
        <div ref={setNodeRef} className={`flex flex-col gap-2 ${styleClassName.memoWidth}`}>
            ...
            <p className={`text-center ${styleClassName.fontMuted} ${styleClassName.fontJustBold}`}>{inProgressStatusToKorean[inProgressStatus]}</p>
                ...
                >
                    {filteredArray.map((progress) => (
                        <SummaryBox key={progress.id} progress={progress} />
                    ))}
                </SortableContext>
            </div>
        )
