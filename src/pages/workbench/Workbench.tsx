...
const CustomButton = React.memo(({ label, status, isSelected, setSelectedStatus }: CustomButtonProps) => {
    ...
    return (
        <Button variant={variant} color={additionalButtonProps.color} sx={additionalButtonProps.sx} onClick={() => setSelectedStatus(status)}>
            {label}
        </Button>
    )
})
    ...
    return (
        <ButtonGroup>
            {BUTTON_CONFIG.map(({ label, status }) => (
                <CustomButton key={status} label={label} status={status} isSelected={selectedStatus === status} setSelectedStatus={setSelectedStatus} />
            ))}
        </ButtonGroup>
    )
}
