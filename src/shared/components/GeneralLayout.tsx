...
const GeneralLayoutSidebar = ({
    ...
    return (
        <div
            ...
            className={`${styleClassName.flexCol} ${
                styleClassName.siebarWidth
            } ${
                doFix
                    ? "overflow-hidden pr-3"
                    : "overflow-x-hidden overflow-y-scroll"
            }  border-r-1 border-black/30 dark:border-white/30 pt-3 pl-3`}
        >
            {children}
        </div>
    )
...
const GeneralLayoutHeader = ({ children }: { children: React.ReactNode }) => {
    return (
        <div
            className={`${styleClassName.flexWide} ${styleClassName.center} border-b-1 border-black/30 dark:border-white/30 p-2`}
        >
            {children}
        </div>
    )
...
const GeneralLayoutBody = ({ children }: { children: React.ReactNode }) => {
    return <div className={`${styleClassName.flex1Fixed} flex`}>{children}</div>
}
