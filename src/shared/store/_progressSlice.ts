...
const createProgressSlice: StateCreator<BoundState, [], [], ProgressSlice> = (
    set
) => ({
    progressArrayInDict: {},
    /** const groupedProgressArray = Object.groupBy(progressArray, (progress) => progress.bookTitle) */
    setProgressArrayInDict(progressArrayInDict) {
        set({ progressArrayInDict })
    },
    ...
    mergeCompletedToInitial() {
        set((state) => {
            return {
                initialCompletedDict: {
                    ...state.initialCompletedDict,
                    ...state.editedCompletedDict,
                },
                editedCompletedDict: {},
            }
        })
    ...
    changeCompleted(progress) {
        set((state) => {
            ...
            progressArrayInDict[progress.bookTitle] = progressArrayInDict[
                progress.bookTitle
            ].map((el) => (el.id === progress.id ? newProgress : el))
    ...
    updateInProgressStatus(bookTitle, id, inProgressStatus) {
        set((state) => {
            const progressArrayInDict = { ...state.progressArrayInDict }
            progressArrayInDict[bookTitle] = progressArrayInDict[bookTitle].map(
                (el) => (el.id === id ? { ...el, inProgressStatus } : el)
            )
    ...
    updateProgress(progress) {
        set((state) => {
            ...
            if (!targetArray) {
                throw new Error("---- ERROR TO PARSE DICT BY PROGRESS!!!!")
            }
            newDict[progress.bookTitle] = newDict[progress.bookTitle].map(
                (el) => (el.id === progress.id ? progress : el)
            )
    ...
    setDoShowSkeleton(doShowSkeleton) {
        set({ doShowSkeleton })
    },
})
